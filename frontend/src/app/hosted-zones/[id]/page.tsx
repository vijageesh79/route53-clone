...
export default function HostedZoneDetailPage() {
  ...
  const [form, setForm] = useState({ name: "", type: "A" as RecordType, ttl: "300", value: "" });
  ...
  }, [zoneId, search, typeFilter, notify]);
  ...
  }, [fetchRecords, auth.loading, auth.user]);
  ...
  useEffect(() => {
    if (editRecord) {
      ...
      setForm({ name: rel, type: editRecord.type as RecordType, ttl: String(editRecord.ttl), value: editRecord.value });
    } else {
  ...
  const handleSaveRecord = async () => {
    setSubmitting(true);
    try {
      const payload = { name: form.name || "@", type: form.type, ttl: Number(form.ttl), value: form.value };
    ...
    <SpaceBetween size="l">
      <Header
        ...
        actions={
          <SpaceBetween direction="horizontal" size="xs">
            {selected.length > 0 && (
              <Button iconName="remove" onClick={() => setDeleteOpen(true)}>Delete ({selected.length})</Button>
            )}
      ...
      <ColumnLayout columns={4} variant="text-grid">
        <Container header={<Header variant="h2">Type</Header>}>
          <StatusIndicator type={zone.type === "Public" ? "info" : "success"}>{zone.type}</StatusIndicator>
        </Container>
      ...
      <Table
        ...
        columnDefinitions={[
          ...
          { id: "policy", header: "Routing policy", cell: (item) => item.routing_policy },
          {
            id: "value",
            header: "Value / Route traffic to",
            cell: (item) => <Box variant="code" fontSize="body-s"><span style={{ whiteSpace: "pre-wrap" }}>{item.value}</span></Box>,
          },
          { id: "ttl", header: "TTL", cell: (item) => item.ttl },
          {
            ...
            header: "Actions",
            cell: (item) => (
              <Button variant="inline-link" onClick={() => setEditRecord(item)}>Edit</Button>
            ),
          },
        ]}
        items={records}
        ...
        filter={
          <SpaceBetween direction="horizontal" size="s">
            <Input type="search" value={search} onChange={({ detail }) => setSearch(detail.value)} placeholder="Filter records" />
            <Select
            ...
            />
            {(search || typeFilter?.value) && <Button onClick={() => { setSearch(""); setTypeFilter(null); }}>Clear</Button>}
          </SpaceBetween>
        }
        header={<Header counter={`(${records.length})`} actions={<Button iconName="refresh" onClick={fetchRecords} />}>Records</Header>}
        empty={<Box textAlign="center"><b>No records</b><Box variant="p">Create or import records to get started.</Box></Box>}
      />

      <Modal
        visible={createOpen || !!editRecord}
        onDismiss={() => { setCreateOpen(false); setEditRecord(null); }}
        header={editRecord ? "Edit record" : "Create record"}
        footer={
          <Box float="right">
            <SpaceBetween direction="horizontal" size="xs">
        ...
        <SpaceBetween size="m">
          ...
          <ColumnLayout columns={2}>
            ...
            <FormField label="TTL (seconds)">
              <Input type="number" value={form.ttl} onChange={({ detail }) => setForm({ ...form, ttl: detail.value })} />
            </FormField>
          </ColumnLayout>
          <FormField label="Value">
            <Textarea value={form.value} onChange={({ detail }) => setForm({ ...form, value: detail.value })} rows={4} />
          </FormField>
        </SpaceBetween>
      </Modal>
